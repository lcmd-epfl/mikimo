...
def add_rate(
    ...
    """
    y_INT, _ = pad_network(y[:np.sum(n_INT_all)], n_INT_all, rxn_network)
    ...
    def bound_decorator(bounds):
        def decorator(func):
            def wrapper(t, y):
                ...
                try:
                    print(isinstance(y, anp.numpy_boxes.ArrayBox))
                    y = y._value
                    dy_dt = dy_dt._value
...
def load_data(args):
    ...
    return initial_conc, Rp, Pp, t_span, temperature, method, energy_profile_all,\
        np.array(dgr_all), coeff_TS_all, rxn_network, n_INT_all
