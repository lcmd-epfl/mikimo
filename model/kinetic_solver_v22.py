...
def load_data(args):
    ...
    species = df_network.columns[1:].tolist()
    nR = len([s for s in species if s.lower().startswith('r') and 'INT' not in s])
    nP = len([s for s in species if s.lower().startswith('p') and 'INT' not in s])
    ...
    Rp, _ = pad_network(rxn_network_all[:n_INT_tot, n_INT_tot:n_INT_tot+nR], n_INT_all, rxn_network)[0]
    Pp, _ = pad_network(rxn_network_all[:n_INT_tot, n_INT_tot+nR:], n_INT_all, rxn_network)[0]
...
def process_data(
    ...
    return k_forward_all, k_reverse_all, Rp_, Pp_
...
if __name__ == "__main__":
    ...
    k_forward_all, k_reverse_all, Rp_, Pp_ = process_data(
        Rp, Pp, energy_profile_all, dgr_all, coeff_TS_all, temperature)
