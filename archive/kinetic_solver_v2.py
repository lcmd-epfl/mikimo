...
def dINTa_dt(
    ...
    for i in range(rxn_network.shape[0]):
        ...
        if cn > 0:
            incr = 0
            if np.all(rxn_network[np.cumsum(n_INT_all)[cn-1]:np.cumsum(n_INT_all)[cn],0] == 0):
                cp_idx = np.where(rxn_network[np.cumsum(n_INT_all)[cn-1]:np.cumsum(n_INT_all)[cn],:][0] == -1)
                tmp_idx = cp_idx[0][0].copy()
                incr += 1
                while tmp_idx != 0:
                    tmp_idx = np.where((rxn_network[tmp_idx,:] == -1))[0][0]
                ...
                for j in range(rxn_network.shape[0]):
                    if j >= np.cumsum(n_INT_all)[cn-1] and j <= np.cumsum(n_INT_all)[cn]: continue
                    else: 
                        if np.any(rxn_network[np.cumsum(n_INT_all)[cn-1]:np.cumsum(n_INT_all)[cn],j]): incr+=1
            a_ = a + mori[cn - 1] - incr
...
def plot_save(result_solve_ivp, rxn_network, Rp_, Pp_):
    ...
    np.savetxt('Ps.txt', result_solve_ivp.y[rxn_network.shape[0] + Rp_[0].shape[1]:])
...
if __name__ == "__main__":
    ...
    k_forward_all, k_reverse_all, Rp_, Pp_, n_INT_all = process_data(
        Rp, Pp, energy_profile_all, dgr_all, coeff_TS_all, rxn_network)
